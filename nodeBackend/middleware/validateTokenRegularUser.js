...
const validateTokenRegularUser = async (req, res, next) => {
    try {
      let token;
      ...
      jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (err, decoded) => {
        ...
        if (decoded.isTrustedUser) {
            logger.info("401: Unauthorized action for trusted users")
            return res.status(401).send({ message: "401: Unauthorized action for trusted users" });
        }
...
module.exports = validateTokenRegularUser;